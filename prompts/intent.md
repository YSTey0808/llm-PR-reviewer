...
- 0-39 benign or standard practice (logging, config reads, documented service calls,
  refactors, dependency bumps). Most PRs land here — and most of THOSE score 0-15,
  not near the top of the band.
...
- If `suspicious_findings` is EMPTY, the change is benign: `risk_score` MUST be 0-39, and
  for ordinary code (refactor, logging, config/env read, dependency bump, docs, tests,
  standard library/API use) it should be 0-15. Do NOT emit 40 as a "nothing jumped out"
  default — 40 is the REVIEW floor and means you found a specific unusual behaviour, which
  you must then name in `suspicious_findings`. No finding ⇒ score below 40.
